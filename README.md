...
- **Обход капчи/проверок Leo:** на любые «проверки»/капчи (сделка, подписка,
  ...
  «Возможно позже»). Срабатывает, когда `UNKNOWN`-сообщение в чате
  Дайвинчика на авто-аккаунте имеет `>= 2` reply-кнопок
  (`CAPTCHA_MIN_BUTTONS`), и не относится к промо «Смотреть анкеты». Так же,
  как кнопка ленты, имеет идемпотентность (не нажимает повторно, если текст
  последней кнопки уже отправлен после карточки) и работает и при старте
