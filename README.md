...
- **Автозапуск потока (безопасный):** `collector.start_auto_stream()` вызывается
  фоном при старте (`main.py`). Он сам гейтится по `enabled` + `start_command`
  (пустая команда → тихо пропускает, ничего вслепую не шлётся).
- **Обработка уже показанной анкеты:** перед отправкой `✨🔍` коллектор сканирует
  последние сообщения чата Leo на авто-аккаунте. Если найдена активная анкета
  (карточка с кнопками `❤️/👎` + ближайший текст анкеты `PROFILE`) — она обрабатывается
  сразу через штатный pipeline (parse → filter → AI → авто-действие), а повторный
  `✨🔍` НЕ отправляется. `✨🔍` уходит только если активной анкеты нет.
...
- **Конфиг (`config/auto_actions`):**
  ...
  auto_actions:
    ...
    start_command: "✨🔍"     # команда открытия потока (если активной анкеты нет)
  ```
