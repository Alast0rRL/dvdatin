...
HARD_NEGATIVE_RULES: list[NegativeRule] = [
    ...
    # Только ОДНОЗНАЧНЫЕ формулировки («ищу только друга/общение»,
    # «не ищу отношения/без отношений/без обязательств»). «Можно пообщаться»,
    # «скучно, пообщаться», «погулять» — обычная открытость в знакомствах и
    # НЕ является признаком отсутствия поиска отношений (ложный DISLIKE).
    NegativeRule(
        code="H01",
        name="not_relationships",
        patterns=[
            "ищу друга", "ищу подругу", "ищу друзей",
            "не ищу отношения", "не ищу отношений", "без отношений",
            "просто ищу общение", "только общение",
            "ищу человечка", "без обязательств",
        ],
...
_NEGATION_PREFIXES = [
    ...
    r"(?:не\s+хочу|не\s+люблю|не\s+даю|не\s+буду|не\s+стала|не\s+старал|не\s+пытал)",
]
...
_THIRD_PERSON_PREFIXES = [
    r"(?<!\w)(?:парень|муж|бойфренд|boyfriend)\s+(?:мой|твой|её|их)",
    r"(?<!\w)(?:парень|муж)\s+",
    r"(?<!\w)(?:он|она|оно)\s+",
