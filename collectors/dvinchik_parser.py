...
# "ᡣ𐭩`,— Брюш .ᐟ.ᐟ, 18, СПб" (декор-префикс с запятой перед именем).
# Имя допускает любой набор символов без перевода строки (включая запятые
# в декоративном префиксе), лениво — возраст ищется по первой запятой после
# которой идёт число и город.
...
# Отказ Leo в лайке/сообщении из-за дневного лимита:
#   «Слишком много ❤️ за сегодня. / Перейди на Premium и получи больше ❤️!»
# Консервативные индикаторы: «слишком много» / «закончились» / «лимит» / «позже»
# в связке с предметом (❤️ / 💌 / лайк / сообщени). Реальная формулировка Leo —
# «слишком много ❤️ за сегодня», но покрываем и вероятные варианты («слишком много
# лайков за сегодня», «лайки закончились», «дневной лимит сообщений»).
REJECTION_AMOUNT_MARKERS = (
    "слишком много",
    "закончились",
    "лимит",
    "позже",
)
REJECTION_SUBJECT_MARKERS = (
    "❤",
    "💌",
    "лайк",
    "сообщени",
...
class DvinchikParser:
    ...
    def is_like_rejection(self, text: str | None) -> bool:
        ...
        True для сообщений вида «Слишком много ❤️ за сегодня. Перейди на Premium
        и получи больше ❤️!» и вероятных вариантов (дневной лимит лайков/
        сообщений). Требуется одновременный признак «нет лимита» И «предмета» —
        описания анкет («слишком общительна», «много хобби») не срабатывают,
        равно как и обычные капчи/меню.
        """
        if not text:
            return False
        lower = text.lower()
        has_amount = any(m in lower for m in REJECTION_AMOUNT_MARKERS)
        if not has_amount:
            return False
        return any(m in lower for m in REJECTION_SUBJECT_MARKERS)
