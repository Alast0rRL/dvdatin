...
- [x] Stage 7.6: «Память капч» — капчу Leo, ответ на которую бот выучил, он пробивает автоматически; неизвестную выводит на сайт `/captchas` (владелец отвечает, ответ запоминается в `captcha_memory` и сразу отправляется Leo). `captcha_signature` = нормализованный текст; опции `enabled` / `auto_answer` / `fallback_press_last`.
- [x] Stage 7.7: Premium-промо после реакции (кнопки [⭐️Активировать, Пока без Premium]) — бот идемпотентно нажимает отказную кнопку (`_press_premium_decline_if_needed`, «Активировать» никогда), лента не встаёт.
