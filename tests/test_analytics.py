...
def setup_basic(stack):
    ...
    aid1 = run(add_ai(db, pid1, "LIKE", combined=0.8))
    ...
    aid2 = run(add_ai(db, pid2, "REVIEW", combined=0.6))
    ...
    aid3 = run(add_ai(db, pid3, "DISLIKE", combined=0.3))
    ...
    run(add_ai(db, pid4, "LIKE", combined=0.9))
...
class TestHistoryExport:
    ...
    def test_export_csv(self, stack, tmp_path: Path) -> None:
        ...
        header = "profile_id,ai_decision,combined_score,confidence," \
                 "human_decision,agreement,scoring_version,created_at,reviewed_at"
    ...
    def test_csv_fields_contract_count_and_order(self) -> None:
        """CSV columns должны быть ровно 9 и в фиксированном порядке."""
