...
> ⚠️ **Правило:** после **каждой новой версии** (любого завершённого коммита)
> деплой и перезапуск ОБЯЗАТЕЛЬНЫ, иначе изменения не считаются применёнными.
> Работа завершается только после `git pull` + `systemctl restart dvai` (+ проверка `is-active`).
