...
- Current counts: test_action_policy (11), test_ai (10), test_analysis_export (10), test_analytics (33), test_audit (27), test_auto_action (63), test_auto_action_audit (5), test_collector (116), test_control_bot (14), test_decision (23), test_deterministic_scoring (84), test_filter (26), test_human_review (23), test_manual_review (13), test_parser (63), test_preferences (12), test_profile (21), test_review_ui (5), test_web (72) → **631 total**. Reset the exact counts from the real file (`tests/baseline/baseline_tests.txt`) when editing them; the summary here is indicative.
...
- Капчи/проверки Leo (сделки, подписки, подтверждения, гео-запросы и т.п.): обработка через `DvinchikCollector._handle_captcha(msg=None)` (заменил `_press_captcha_button`). Определение капчи НЕ изменилось: текст должен содержать один из маркеров `CAPTCHA_MARKERS` (сделк/подписываешься/подтверд/верификац/расположени/геолокаци/координат и т.п.), а reply-кнопок должно быть `>= CAPTCHA_MIN_BUTTONS` (иначе зациклимся в меню/Premium-промо). **Память капч (Stage 7.6)**: сигнатура капчи = `captcha_signature(text)` (нижний регистр + схлопнутые пробелы) → таблица `captcha_memory`. Выученный ответ (`get_captcha_answer`) → авто-отправка через `_send_captcha_text` (идемпотентно: повторно не шлём, если текст уже отправлялся после карточки капчи); используется `mark_captcha_used`. Неизвестная капча → `record_pending_captcha` (pending, answer NULL) и вывод на сайт `/captchas`; кнопку наугад НЕ нажимаем (ждём ответа владельца) — кроме включённого `auto_actions.captcha.fallback_press_last` (тогда старое поведение: нажимается последняя ОБЫЧНАЯ кнопка — спец-кнопки гео/телефон текстом не нажимаются, в гео-капче берётся «Продолжить смотреть анкеты» = отказ от координат). `captcha.enabled=false` → полностью старое поведение (последняя кнопка, без памяти). Работает и при старте (`start_auto_stream` fallback после view-кнопки), и в live-обработке (`UNKNOWN`-ветка → `_handle_captcha(msg)`). Конфиг — `auto_actions.captcha: enabled / auto_answer / fallback_press_last`.
...
- **Premium-промо после реакции (Stage 7.7)**: когда после ❤️/👎 Leo присылает премиум-рекламу («Борюсь с эгоизмом…», кнопки [⭐️Активировать, Пока без Premium]), лента без нажатия встаёт. `DvinchikCollector._press_premium_decline_if_needed()` сканирует последние 15 сообщений, находит самую свежую карточку с кнопкой, содержащей `PREMIUM_DECLINE_FRAGMENT` («без premium»), и нажимает отказную кнопку (идемпотентно, как view-кнопка; «Активировать» — никогда). Подключён в `start_auto_stream` (после view-кнопки) и в live-ветки `UNKNOWN`/`SERVICE` (иначе бот реагирует на анкеты и встаёт на премиум-рекламе). Текст отказа («Пока без Premium») исключён из `sent_messages`-аналитики (`is_manual_message_excluded`).
...
- **Результат фиксируется на КАЖДЫЙ лайк**: таблица `like_outcomes` — по лайку (авто/ручной) хранит `message_text` (содержание сообщения, '' если без текста) и `responded` (0/1) — лайкнула девушка в ответ или нет. Рядом с каждым лайком (`LIKE` в авто-журнале или ручном `sent_messages`) заводится строка через `record_auto_action`/`record_sent_message`; `MESSAGE` прикрепляет текст к лайку на ту же карточку/время; взаимный лайк (`record_match_response`) проставляет `responded=1` по всем висящим лайкам профиля (только `rejected=0`). Идемпотентность — UNIQUE(`source, chat_id, like_tm_id`). Бэкфилл старых лайков из `auto_actions_log`∪`sent_messages` в `_ensure_like_outcomes` (INSERT OR IGNORE).
- **Отклонённые лайки (лимит Leo)**: если Leo на ❤️ отвечает «Слишком много ❤️ за сегодня» (детектор `DvinchikParser.is_like_rejection` — маркеры «слишком много»/«закончились»/«лимит»/«позже» + предмет «❤/💌/лайк/сообщени»), коллектор вызывает `Database.mark_like_rejected(chat_id)` — свежий неотклонённый лайк чата (окно 120 c) помечается `rejected=1` и из аналитики (`get_top_liked_profiles`, флипы `responded`) исключается. Повторные отказы подряд помечают лайки по очереди. Само сообщение трактуется как SERVICE. Колонка `rejected` мигрируется в `_ensure_like_outcomes` (ALTER ДО создания `idx_like_outcomes_rejected` — в top-SCHEMA его нет, иначе legacy-базы падали бы на executescript).
- **Два источника сообщений при лайке**:
  ...
  2. Ручные действия владельца — фиксируются в `sent_messages` (`source='manual'`) с классификацией `action`: «❤️»→**LIKE** (сам лайкнул), «👎»→**DISLIKE**, остальные тексты→**MESSAGE**. Запись — в `_handle_outgoing_message` → `_maybe_record_sent_message`. Исходящие с авто-аккаунта пропускаются ТОЛЬКО когда авто-действия реально включены (`auto_engine.enabled`, SEMI_AUTO/AUTO) — они уже в `auto_actions_log`; в OBSERVE авто ничего не шлёт, поэтому любой исходящий (в т.ч. с авто-аккаунта) = ручное действие владельца и записывается. Кнопки/навигация Leo отсекаются через `is_manual_message_excluded` (`MANUAL_MESSAGE_EXCLUDE` + `MANUAL_MESSAGE_EXCLUDE_FRAGMENTS` + нумерованные кнопки меню «1 🚀»/«1 👍» по regex, тексты короче 2 символов). Профиль привязывается по «текущей» анкете чата (`get_chat_profile_context`/`_pending_profiles` → `Database.resolve_profile_by_message`).
- **Сигнал ответа** — взаимный лайк: MATCH-сообщение «Начинай общаться 👉 [Имя]» → `Database.record_match_response(name, chat_id, tm_id, telegram_username=...)`. Привязка к профилю по имени (`_resolve_profile_by_name`): регистронезависимо по-настоящему (Python `lower`), приоритет — профили со свежими записями `auto_actions_log`∪`sent_messages`. Дубликаты на карточку отсекаются UNIQUE-индексом `(chat_id, telegram_message_id)` (INSERT OR IGNORE). Заодно проставляет `responded=1` в `like_outcomes`.
- **Отчёты** (Telegram-free, `AnalyticsService` + `Database`): `get_message_response_stats()` — по каждому тексту `sent` (уникальных профилей) / `responded` / `rate`; `get_top_liked_profiles(limit)` — топ по лайкам (из `like_outcomes`, авто∪ручные) + ответили ли + последнее сообщение при лайке. Команды ControlBot `/msgs` и `/top` (+ inline-кнопки «💬 Сообщения»/«💖 Топ лайков»).
