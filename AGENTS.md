...
- **Два источника сообщений при лайке**:
  ...
  2. Ручные действия владельца — фиксируются в `sent_messages` (`source='manual'`) с классификацией `action`: «❤️»→**LIKE** (сам лайкнул), «👎»→**DISLIKE**, остальные тексты→**MESSAGE**. Запись — в `_handle_outgoing_message` → `_maybe_record_sent_message`. НЕ записываются: исходящие с авто-аккаунта (`msg.client is auto_engine.client` — уже в авто-журнале) и кнопки/навигация Leo через `is_manual_message_excluded` (`MANUAL_MESSAGE_EXCLUDE` + `MANUAL_MESSAGE_EXCLUDE_FRAGMENTS` + нумерованные кнопки меню «1 🚀»/«1 👍» по regex, тексты короче 2 символов). Профиль привязывается по «текущей» анкете чата (`get_chat_profile_context`/`_pending_profiles` → `Database.resolve_profile_by_message`).
